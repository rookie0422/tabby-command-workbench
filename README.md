...
```bash
git clone https://github.com/rookie0422/tabby-serial-command-sidebar.git
cd tabby-serial-command-sidebar
npm ci
npm run check
```
...
包名 `tabby-serial-command-sidebar` 发布在公共 npm Registry。
...
创建 GitHub Release 后，`.github/workflows/publish.yml` 会自动发布对应版本。

发布工作流支持 npm Trusted Publishing。首次发布包后，可在 npm 包设置中配置：

- Provider: GitHub Actions
- Repository owner: `rookie0422`
- Repository: `tabby-serial-command-sidebar`
- Workflow: `publish.yml`

配置完成后无需保存长期 npm Token。若尚未配置 Trusted Publishing，也可以在 GitHub 仓库的 Actions secrets 中添加 `NPM_TOKEN`。
